...
namespace Epsilon::OpenGL
{
    ...
    public:
        ...
        vbo_(0), ebo_(0)
        {
          ...
          glVertexAttribPointer(1, 2, GL_FLOAT, false, 2 * sizeof(float), reinterpret_cast<void*>(offsetof(Vertex, uv)));
          glEnableVertexAttribArray(1);

          //normal coordinates will be on 2
          glVertexAttribPointer(2, 3, GL_FLOAT, false, 3 * sizeof(float), reinterpret_cast<void*>(offsetof(Vertex, normal)));
          glEnableVertexAttribArray(2);

          //index buffer
          glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo_);
          glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(sizeof(uint64_t) * indices_.size()), indices_.data(), GL_DYNAMIC_DRAW);
